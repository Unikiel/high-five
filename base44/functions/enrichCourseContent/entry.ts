...
function calculusFocus(title) {
  const t = title.toLowerCase();
  if (t.includes('limit')) return { area: 'limits and limiting behavior', formulas: ['\\lim_{x\\to a} f(x)=L', '\\lim_{x\\to\\infty} f(x)=L'], method: 'analyze values as the input approaches a point or infinity, using graphs, tables, algebra, or the Squeeze Theorem' };
  if (t.includes('continu')) return { area: 'continuity', formulas: ['\\lim_{x\\to a} f(x)=f(a)', 'f(c)=N\\text{ for some }c\\in(a,b)'], method: 'verify function value, limit existence, and equality between the two; use IVT only on closed intervals with continuity' };
  if (t.includes('derivative') || t.includes('differentiab') || t.includes('rate')) return { area: 'derivatives and rates of change', formulas: ["f'(a)=\\lim_{h\\to0}\\frac{f(a+h)-f(a)}{h}", '\\frac{dy}{dx}', '\\frac{d}{dx}x^n=nx^{n-1}'], method: 'interpret the derivative as instantaneous rate of change, slope of a tangent line, or sensitivity in context' };
  if (t.includes('chain')) return { area: 'the Chain Rule', formulas: ['\\frac{d}{dx}f(g(x))=f\'(g(x))g\'(x)'], method: 'differentiate the outside function while preserving the inside, then multiply by the derivative of the inside' };
  ...
  if (t.includes('related rates')) return { area: 'related rates', formulas: ['\\frac{d}{dt}V', '\\frac{d}{dt}A', '\\frac{dy}{dt}=\\frac{dy}{dx}\\frac{dx}{dt}'], method: 'write an equation relating changing quantities, differentiate with respect to time, then substitute known values' };
  if (t.includes('l\'hospital')) return { area: "L'Hospital's Rule", formulas: ['\\lim\\frac{f(x)}{g(x)}=\\lim\\frac{f\'(x)}{g\'(x)}'], method: 'first confirm an indeterminate form such as 0/0 or infinity/infinity, then differentiate numerator and denominator separately' };
  ...
  if (t.includes('optimization')) return { area: 'optimization', formulas: ["f'(c)=0\\text{ or undefined at candidates}"], method: 'define the target function, restrict the domain, find candidates, and compare values in context' };
  if (t.includes('riemann') || t.includes('definite integral') || t.includes('accumulation')) return { area: 'definite integrals and accumulation', formulas: ['\\int_a^b f(x)\\,dx', '\\sum f(x_i^*)\\Delta x'], method: 'interpret signed area and accumulated change using units and interval bounds' };
  ...
  if (t.includes('antiderivative') || t.includes('substitution') || t.includes('integrating')) return { area: 'antiderivatives and integration techniques', formulas: ['\\int x^n dx=\\frac{x^{n+1}}{n+1}+C', '\\int f(g(x))g\'(x)dx'], method: 'select an antiderivative rule or substitution that reverses the derivative structure' };
  if (t.includes('differential equation') || t.includes('slope field') || t.includes('euler')) return { area: 'differential equations', formulas: ['\\frac{dy}{dx}=f(x,y)', 'y_{n+1}=y_n+f(x_n,y_n)\\Delta x'], method: 'represent a relationship between a function and its derivative, then solve, approximate, or interpret the solution' };
  if (t.includes('area between')) return { area: 'area between curves', formulas: ['\\int_a^b(upper-lower)\\,dx', '\\int_c^d(right-left)\\,dy'], method: 'identify the bounding curves, choose x- or y-slices, and integrate top-minus-bottom or right-minus-left' };
  if (t.includes('volume') || t.includes('washer') || t.includes('disc') || t.includes('cross section')) return { area: 'volumes from cross sections or rotation', formulas: ['V=\\int_a^b A(x)\\,dx', 'V=\\pi\\int_a^b(R^2-r^2)\\,dx'], method: 'write the cross-sectional area or washer/disc area as a function of the slicing variable' };
  if (t.includes('parametric')) return { area: 'parametric equations', formulas: ['\\frac{dy}{dx}=\\frac{dy/dt}{dx/dt}', '\\frac{d^2y}{dx^2}=\\frac{d}{dt}(dy/dx)\\big/\\frac{dx}{dt}'], method: 'differentiate x(t) and y(t) with respect to the parameter and convert to dy/dx when needed' };
  if (t.includes('vector')) return { area: 'vector-valued functions', formulas: ['\\vec r(t)=\\langle x(t),y(t)\\rangle', '\\vec v(t)=\\vec r\'(t)', '\\vec a(t)=\\vec v\'(t)'], method: 'treat position, velocity, and acceleration component-wise and interpret motion from vector quantities' };
  if (t.includes('polar')) return { area: 'polar functions', formulas: ['A=\\frac12\\int_\\alpha^\\beta r^2\\,d\\theta', '\\frac{dy}{dx}=\\frac{dy/d\\theta}{dx/d\\theta}'], method: 'convert polar information into area, slope, or motion relationships using r and theta' };
  if (t.includes('series') || t.includes('convergen') || t.includes('taylor') || t.includes('maclaurin') || t.includes('power')) return { area: 'sequences, series, and power series', formulas: ['\\sum_{n=1}^{\\infty}a_n', '\\sum ar^n=\\frac{a}{1-r}', 'f(x)=\\sum_{n=0}^{\\infty}\\frac{f^{(n)}(a)}{n!}(x-a)^n'], method: 'determine convergence, estimate error, or represent functions with Taylor and power series' };
  return { area: 'calculus reasoning', formulas: ["f'(x)", '\\int_a^b f(x)\\,dx'], method: 'choose the relevant derivative, integral, limit, or series representation and justify it with AP-ready notation' };
}
