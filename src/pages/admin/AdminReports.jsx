...
export default function AdminReports() {
  ...
  const totalStudyMinutes = progress.reduce((s, p) => s + (p.time_spent_minutes || 0), 0);
  ...
  const avgCompletionPct =
    progress.length > 0 ? Math.round((completedProgress / progress.length) * 100) : null;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Platform-wide performance and engagement analytics
          </p>
        </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              ...
              },
              {
                ...
                value: avgCompletionPct != null ? `${avgCompletionPct}%` : "—",
                icon: BookOpen,
                color: "text-pink-500",
                ...
                {filteredExams.length === 0 ? (
                  ...
                  <div className="space-y-3 pt-2">
                    {scoreRanges.map(({ range, count }) => {
                      const pct =
                        filteredExams.length > 0
                          ? Math.round((count / filteredExams.length) * 100)
                          : 0;
                      return (
                        <div key={range}>
                  ...
                  .map((s) => {
                    ...
                    const avg =
                      studentExams.length > 0
                        ? Math.round(
                            studentExams.reduce((acc, e) => acc + (e.score || 0), 0) /
                              studentExams.length
                          )
                        : 0;
